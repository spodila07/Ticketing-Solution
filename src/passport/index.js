        ...
        User.findOne({ username: new RegExp('^' + username.trim() + '$', 'i') })
          ...
          .exec(function (err, user) {
            ...
            if (!user || user.deleted || !User.validate(password, user.password)) {
             ...
             // Existing PassportJS logic for invalid login
             return done(null, false, req.flash('loginMessage', `<div style="margin: 0 auto; text-align: center; background-color: white; color: orange; border: 2px solid orange; padding: 10px; width: 300px; border-radius: 8px;">Invalid <strong>Username/Password</strong></div>`));
            }
            req.user = user
